    ...
    public class Application_Operations: IApplication_Operations
    {
                    ...
                    using (var transaction = await context.Database.BeginTransactionAsync())
                    {
                        ...
                        try
                        {
                            ...
                            var application = new Application
                            {
                                ...
                                ApplicationStatus_ID = applicationStatusId,//Status must be Submitted
                                Grade_ID = gradeId,
        ...
        public async Task<List<Application>> GetApplicationsByApplicantId(Int64 applicantId)
        {
            try
            {
                using (var context = new DatabaseContext(DatabaseContext.Options.DatabaseOptions))
                {
                    List<Application> Applications = await context.Applications.Include(a => a.Applicant).OrderBy(a => a.SchoolYear).Where(a => a.Applicant_ID == applicantId).ToListAsync();
