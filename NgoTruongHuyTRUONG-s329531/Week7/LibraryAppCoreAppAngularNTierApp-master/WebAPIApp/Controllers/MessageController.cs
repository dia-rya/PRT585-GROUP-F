...
namespace WEB_API.Controllers
{
    ...
    public class MessageController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("[action]")]
        public async Task<IActionResult> AddMessage(string name)
        {
            var result = await _Message_Service.AddMessage(name);
        ...
        {
            var result = await _Message_Service.UpdateMessage(message.id, message.name);
            switch (result.success)
            {
                case true:
                    return Ok(result);
